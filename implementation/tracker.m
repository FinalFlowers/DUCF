...
while true
    ...
    if seq.frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            xt = extract_features(im, sample_pos, sample_scale, features, global_fparams, feature_extract_info, seq);
  ...
  %========================================DUCF==========================================
    if seq.frame == 1
        seqRow = size(im,1);
        seqCol = size(im,2);
        lastImage = im(min(max(1,round(pos(1)-0.5*target_sz(1))),seqRow):...
                       min(max(1,round(pos(1)+0.5*target_sz(1))),seqRow),...  %Ŀ����ͼ���ϱ�Ե���±�Ե֮��
                       min(max(1,round(pos(2)-0.5*target_sz(2))),seqCol):...
                       min(max(1,round(pos(2)+0.5*target_sz(2))),seqCol),...  %Ŀ����ͼ�����Ե���ұ�Ե֮��?
                       :);
        imageRow = size(lastImage,1);
        imageCol = size(lastImage,2);
    else
        seqRow = size(im,1);
        seqCol = size(im,2);
        currentImage = im(min(max(1,round(pos(1)-0.5*target_sz(1))),seqRow):...
                          min(max(1,round(pos(1)+0.5*target_sz(1))),seqRow),...  %Ŀ����ͼ���ϱ�Ե���±�Ե֮��
                          min(max(1,round(pos(2)-0.5*target_sz(2))),seqCol):...
                          min(max(1,round(pos(2)+0.5*target_sz(2))),seqCol),...  %Ŀ����ͼ�����Ե���ұ�Ե֮��?
                          :);
        ...
        currentImageResize = imresize(currentImage,[imageRow imageCol]);

        if is_color_image
            lastImageGray = double(rgb2gray(lastImage));
            currentImageResizeGray = double(rgb2gray(currentImageResize));
        else
            lastImageGray = double(lastImage);
            currentImageResizeGray = double(currentImageResize);
        end

        [vx,vy] = HS(lastImageGray,currentImageResizeGray);

        clear flow;
        flow(:,:,1) = vx;
        flow(:,:,2) = vy;

        maxFlow = flowStat(flow);
        is_need = libsvmTest(flow,model);
...
%     params.train_gap = 0;%�����������? 2019.3.17 �ǵ��յ�����ʱ����ע��
...
%==============================================================================
    ...
    if seq.frame == 1
        ...
        xl = extract_features(im, sample_pos, currentScaleFactor, features, global_fparams, feature_extract_info, seq);
...
%             set(fig_handle, 'Position', [100, 100, size(im,2), size(im,1)]);
            ...
            text(50,10,['Ns=' int2str(params.train_gap+1)], 'color', [0 1 1]);
            hold off;
