...
if nargin<1 || nargin<2
    n=103;
    addpath('J:\毕业设计\程序\ECO-master\ECO-master\sequences\MotorRollingObject');
%     im1=imread('yos9.tif');
%     im2=imread('yos10.tif');
    G = fspecial('gaussian', [5 5], 2);

    Image1 = imread([int2str(n),'.jpg']);
    Image2 = imread([int2str(n+1),'.jpg']);
    rowSize = size(Image1,1);
    colSize = size(Image1,2);
    Image2 = imresize(Image2,[rowSize,colSize]);
    
    Image1 = imfilter(Image1,G,'same');
    Image2 = imfilter(Image2,G,'same');
    im1 = Image1;
    im2 = Image2;
